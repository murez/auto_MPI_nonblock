...
is_mpi_collective_call(CallInst *CI) {
  std::vector<std::string> mpi_collective_calls = {"MPI_Allgather",
                                                   "MPI_Allgatherv",
                                                   "MPI_Allreduce",
                                                   "MPI_Alltoall",
                                                   "MPI_Alltoallv",
...
void replace_mpi_coll_f(Module &M) {
  for (auto &F : M) {
    for (auto &BB : F) {
      for (auto &I : BB) {
        if (auto *CI = dyn_cast<CallInst>(&I)) {
          auto [is_mpi_coll, func_name, nonblock_func_name] =
              is_mpi_collective_call(CI);
          if (is_mpi_coll) {
            ...
            AllocaInst *mpi_req = new AllocaInst(
                Int32Ty, 0, mpi_req_size, name_id("mpi_req", rand_id), CI);
            AllocaInst *mpi_status = new AllocaInst(
                Int32Ty, 0, mpi_req_size, name_id("mpi_status", rand_id), CI);
            ...
            CallInst *MPI_Wait_Call =
                CallInst::Create(MPI_Wait_Func, {mpi_req, mpi_status});
            MPI_Wait_Call->setName(name_id("MPI_Wait", rand_id));
            MPI_Wait_Call->insertAfter(MPI_I_Call);
            // mark this CI to be deleted

            errs() << "Created " << nonblock_func_name
                   << " and MPI_Wait for ID: [" << rand_id << "]\n";
